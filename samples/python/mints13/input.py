...
def test_dimer(mol, expected_cgmp, label, mtype):
    ...
    qcdb.compare_molrecs(dAB, mAB.to_dict(), label + " AB roundtrip", atol=1.0e-6)
    ...
    qcdb.compare_molrecs(dA_, mA_.to_dict(), label + " A_ roundtrip", atol=1.0e-6)

    _B = mol.extract_subsets(2)
    d_B = _B.to_dict()
    test_chgmult(expected_cgmp["_B"], d_B, label + " _B")
    m_B = mtype.from_dict(d_B)
    qcdb.compare_molrecs(d_B, m_B.to_dict(), label + " _B roundtrip", atol=1.0e-6)
