...
void formInvSqrtJ(double **&J_mhalf, shared_ptr<BasisSet> basis, shared_ptr<BasisSet> ribasis,
                  shared_ptr<BasisSet> zero) {
    // Create integral factories for the RI basis
    shared_ptr<IntegralFactory> rifactory_J(new IntegralFactory(ribasis, zero, ribasis, zero));
    ...
    int status = C_DSYEV('v', 'u', ribasis->nbf(), J[0], ribasis->nbf(), eigval, work, lwork);
    if (status) {
    ...
    // J_mhalf = J_copy(T) * J
    C_DGEMM('t', 'n', ribasis->nbf(), ribasis->nbf(), ribasis->nbf(), 1.0, J_copy[0], ribasis->nbf(), J[0],
            ribasis->nbf(), 0.0, J_mhalf[0], ribasis->nbf());
    free_block(J);
    free_block(J_copy);
