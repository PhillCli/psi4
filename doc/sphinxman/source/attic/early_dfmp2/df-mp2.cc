...
extern void formInvSqrtJ(double **&J_mhalf, shared_ptr<BasisSet> basis, shared_ptr<BasisSet> ribasis,
                         shared_ptr<BasisSet> zero);
...
PsiReturnType dfmp2(Options &options, int argc, char *argv[]) {
    ...
    if (ri_basis == "") {
        throw InputException(std::string("Keyword not specified"), std::string("RI_BASIS"), __FILE__, __LINE__);
    }
    std::string orbital_basis = options.get_str("BASIS");
    if (orbital_basis == "") {
        throw InputException(std::string("Keyword not specified"), std::string("BASIS"), __FILE__, __LINE__);
    }
    ...
    fprintf(outfile, "\t\t----------------------------------------------\n");
    fprintf(outfile, "\t\t  %5d  %5d  %4d %4d %4d %4d %4d %4d\n", norbs, ribasis->nbf(), nfocc, ndocc, nact_docc,
            nact_virt, nvirt, nfvir);
    fprintf(outfile, "\t\t==============================================\n");

    // Read in MO coefficients
    shared_ptr<SimpleMatrix> C_so(factory->create_simple_matrix("MO coefficients (SO basis)"));

    double *orbital_energies = chkpt->rd_evals();
    double **vectors = chkpt->rd_scf();

    if (vectors == NULL) {
        throw PsiException("Could not find MO coefficients. Run cscf first.", __FILE__, __LINE__);
    }
    C_so->set(vectors);
    free_block(vectors);
    double **Umat = chkpt->rd_usotbf();
    shared_ptr<SimpleMatrix> U(factory->create_simple_matrix("SO->BF"));
    U->set(Umat);
    free_block(Umat);
    // Transfrom the eigenvectors from the SO to the AO basis
    shared_ptr<SimpleMatrix> C(factory->create_simple_matrix("MO coefficients (AO basis)"));
    ...
    shared_ptr<IntegralFactory> rifactory(new IntegralFactory(ribasis, zero, basis, basis));
    ...
    for (int Pshell = 0; Pshell < ribasis->nshell(); ++Pshell) {
        ...
        for (int P = 0, index = 0; P < numPshell; ++P) {
            ...
            // Do transform
            C_DGEMM('T', 'N', nact_docc, norbs, norbs, 1.0, Co[0], nact_docc, temp[P][0], norbs, 0.0, half[0], norbs);
            C_DGEMM('N', 'N', nact_docc, nact_virt, norbs, 1.0, half[0], norbs, Cv[0], nact_virt, 0.0, mo_p_ia[oP],
                    nact_virt);
        }
    ...
    C_DGEMM('T', 'N', nact_docc * nact_virt, ribasis->nbf(), ribasis->nbf(), 1.0, mo_p_ia[0], nact_docc * nact_virt,
            J_mhalf[0], ribasis->nbf(), 0.0, B_ia_p[0], ribasis->nbf());
        ...
        for (int j = 0; j <= i; ++j) {
            ...
            for (int a = 0, ab = 0; a < nact_virt; ++a) {
                for (int b = 0; b < nact_virt; ++b, ab++) {
                    ...
                    ibja = I[ba];
                    denom =
                        1.0 / (epsilon_act_docc[i] + epsilon_act_docc[j] - epsilon_act_virt[a] - epsilon_act_virt[b]);
