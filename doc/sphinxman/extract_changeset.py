...
log_cmd = (
    "git log --branches=master -%s --no-merges --pretty=format:'<title> %%cr [%%h] by %%cn: <br/> %%s<br/></title>' > %s"
    % (Nhist, writedir + os.sep + githist)
)
os.system(log_cmd)

log_cmd = (
    "git log --branches=master -1 --no-merges --pretty=format:'<title> %%cr by %%cn: <br/> %%s<br/></title>' > %s"
