...
def reconcile_and_compute_version_output(quiet=False):
    ...
    sane_tag = re.compile(
        r"""^(?P<tag>(?P<forwardseries>\d+\.\d+(?P<patch>\.[1-9]+)?)(?(patch)|(?P<prere>((a)|(b)|(rc))\d+)?))$"""
    )
        ...
        if res["latest_annotated_v_tag"] == meta_latest_annotated_v_tag:
            ...
            if int(res["commits_since_tag"]) == 0:
                ...
                # TODO prob should be undef unless on master
                    ...
                    if not quiet:
                        print(
                            """Defining development snapshot version: {} (computed)""".format(trial_version_long_devel)
                        )
...
def version_formatter(versdata, formatstring="""{version}"""):
    ...
    release = (
        "release" if versdata["__version_release"] else ("prerelease" if versdata["__version_prerelease"] else "")
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to extract Psi4 version from source. Use psi4.version_formatter(fmt_string) after build."
    )
