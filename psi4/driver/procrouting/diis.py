...
class DIIS:
        ...
        if isinstance(template_object, float) or self.storage_policy == StoragePolicy.InCore:
            ...
            if hasattr(template_object, "__len__"):
                ...
                if template_object == ambit.BlockedTensor:
                    quantity = ambit.BlockedTensor.load_and_build(
                        f"{core.IOManager.shared_object().get_default_path()}/libdiis.{full_name}"
                    )
        else:
    ...
    def build_entry(self, entry, target_index):
        return {
            key: [self.copier(elt, self.get_name(key, target_index, i)) for i, elt in enumerate(val)]
