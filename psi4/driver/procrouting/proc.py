...
def scf_helper(name, post_scf=True, **kwargs):
    ...
    if core.get_option('SCF', 'DDX'):
        ...
        scf_wfn.ddx_state = solvent.ddx.DdxInterface(molecule=scf_molecule,
                                                     options=ddx_options,
    ...
    if core.get_option('SCF', 'PE'):
        if not solvent._have_pe:
            raise ModuleNotFoundError(
                'Python module cppe not found. Solve by installing it: `conda install -c psi4 pycppe`')
...
def run_scf_hessian(name, **kwargs):
    ...
    if badref or badint:
        raise ValidationError(
            "Only RHF/UHF/RKS Hessians are currently implemented. SCF_TYPE either CD or OUT_OF_CORE not supported")
