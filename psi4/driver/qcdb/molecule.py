...
from .testing import compare_integers, compare_molrecs, compare_values
...
class Molecule(LibmintsMolecule):
    ...
    def print_ring_planes(self, entity1, entity2, entity3=None, entity4=None):
        ...
        for item in [entity1, entity2]:
            ...
            dtemp = math.sqrt(
                evecs[0][midx] * evecs[0][midx] + evecs[1][midx] * evecs[1][midx] + evecs[2][midx] * evecs[2][midx]
            )
        ...
        if summ[0]["dim"] == "plane" and (summ[1]["dim"] == "plane" or summ[1]["dim"] == "point"):
            ...
            text += "  Distance from Center of %s to Center of %s:                   %14.8f   [Angstrom]\n" % (
                "2",
                "1",
                distCC * constants.bohr2angstroms,
            )
            ...
            text += "  Distance from Center of %s to Plane of %s:                    %14.8f   [Angstrom]\n" % (
                "2",
                "1",
                distCP * constants.bohr2angstroms,
            )
            ...
            text += "  Distance from Center of %s to Center of %s along Plane of %s:  %14.8f   [Angstrom]\n" % (
                ...
                "1",
                "1",
                distCPC * constants.bohr2angstroms,
            )
    ...
    def rotor_type(self, tol=FULL_PG_TOL):
        ...
        rot_const = [
            1.0 / evals[0] if evals[0] > 1.0e-6 else 0.0,
    ...
    def to_arrays(
        self, dummy: bool = False, ghost_as_dummy: bool = False
        ...
        if dummy:
            ...
            mass = np.asarray(
                [(0.0 if (ghost_as_dummy and self.fZ(at) == 0) else self.fmass(at)) for at in range(self.nallatom())]
            )
            elem = np.asarray(
    ...
    def run_dftd3(
        ...
        if derint == -1:
            return (
                float(jobrec["extras"]["qcvars"]["DISPERSION CORRECTION ENERGY"]),
    ...
    def run_gcp(self, func: Optional[str] = None, dertype: Union[int, str, None] = None, verbose: int = 1):
        ...
        if derint == -1:
            return (
                float(jobrec["extras"]["qcvars"]["GCP CORRECTION ENERGY"]),
    ...
    def to_dict(self, force_c1=False, force_units=False, np_out=True):
        ...
        if force_units == "Bohr":
            ...
            units = self.units()
            molrec["units"] = units
            if units == "Angstrom" and abs(self.input_units_to_au() * constants.bohr2angstroms - 1.0) > 1.0e-6:
        ...
        try:
            ...
            validated_molrec = qcel.molparse.from_arrays(
                speclabel=False, nonphysical=True, verbose=0, domain="qm", **molrec
            )
            forgive.append("fragment_charges")
            forgive.append("fragment_multiplicities")
        compare_molrecs(validated_molrec, molrec, "to_dict", atol=1.0e-6, forgive=forgive, verbose=0)
    ...
    def _internal_from_dict(self, molrec, verbose=1):
        ...
        if "geom_unsettled" in molrec:
            ...
            for iat in range(nat):
                ...
                self.add_unsettled_atom(
                    Z, entry, molrec["elem"][iat], molrec["mass"][iat], Z, label, molrec["elea"][iat]
                )
    ...
    def B787(
        concern_mol: Union[qcdbmol, psi4.core.Molecule],
