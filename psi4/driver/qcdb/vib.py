...
def compare_vibinfos(
    expected: Dict[str, Datum],
    ...
    svdtol = 1.0e-6 if toldict is None else toldict.get("svd", 1.0e-6)
...
def print_molden_vibs(
    vibinfo: Dict[str, Datum],
...
def harmonic_analysis(
    ...
    uconv_kmmol = (
        constants.get("Avogadro constant")
        * np.pi
        * 1.0e-3
        * constants.get("electron mass in u")
        * constants.get("fine-structure constant") ** 2
        * constants.get("atomic unit of length")
        / 3
    )
    uconv_D2A2u = (
        constants.get("atomic unit of electric dipole mom.")
        * 1.0e11
    ...
    turning_point_bohr_u[turning_point_bohr_u == np.inf] = 0.0
    ...
    turning_point_bohr[turning_point_bohr == np.inf] = 0.0
...
def thermo(
    ...
    q_trans = (
        (2.0 * np.pi * molecular_mass * constants.amu2kg / (beta * constants.h * constants.h)) ** 1.5
        * constants.na
        / (beta * P)
    )
    ...
    if rotor_type == "RT_ATOM":
        ...
        q_rot = 1.0 / (beta * sigma * 100 * constants.c * constants.h * rot_const[1])
    ...
    lowfreqidx = np.where(filtered_theta_vib < 900.0)[0]
    if len(lowfreqidx):
        print(
            "Warning: used thermodynamics relations inappropriate for low-frequency modes: {}".format(
    ...
    uconv = np.asarray([constants.hartree2kcalmol, constants.hartree2kJmol, 1.0])
...
def filter_nonvib(vibinfo: Dict[str, Datum], remove: List[int] = None) -> Dict[str, Datum]:
    ...
    for asp, oasp in vibinfo.items():
        if asp in ["q", "w", "x"]:
            ...
            axis = 0
        work[asp] = Datum(
            oasp.label, oasp.units, np.delete(oasp.data, remove, axis=axis), comment=oasp.comment, numeric=False
        )
...
def _get_TR_space(
    m: np.ndarray, geom: np.ndarray, space: str = "TR", tol: float = None, verbose: int = 1
) -> np.ndarray:
