...
def harvest_output(outtext):
    ...
    mobj = re.search(
        r"^\s*"
        + r"Reference energy[:]?\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"MP2 singlet pair energy[:]?\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"MP2 triplet pair energy[:]?\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"MP2 correlation energy[:]?\s+"
        + NUMBER
        + r"\s*$",
        outtext,
        re.MULTILINE,
    ...
    if mobj:
        ...
        psivar["MP2 SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(3)) * Decimal(2) / Decimal(3)
        psivar["MP2 OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(4)) - psivar["MP2 SAME-SPIN CORRELATION ENERGY"]
        )
    ...
    mobj = re.search(
        r"^\s+"
        + r"CABS-singles contribution of\s+"
        + NUMBER
        + r"\s+patched into reference energy.\s*"
        + r"^\s+"
        + r"New reference energy\s+"
        + NUMBER
        + r"\s*$",
        outtext,
        re.MULTILINE,
    ...
    mobj = re.search(
        r"^\s*"
        + r"DF-MP2-F12 correlation energies:\s*"
        + r"^\s*(?:[=-]+)\s*"
        + r"^\s+"
        + r"Approx.\s+Singlet\s+Triplet\s+Ecorr\s+Total Energy\s*"
        + r"^\s+"
        + r"DF-MP2\s+"
        + NUMBER
        + r"\s+"
    ...
    if mobj:
        ...
        psivar["MP2 SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(2)) * Decimal(2) / Decimal(3)
        psivar["MP2 OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(3)) - psivar["MP2 SAME-SPIN CORRELATION ENERGY"]
        )
        psivar["MP2 TOTAL ENERGY"] = mobj.group(4)
        psivar["MP2-F12 CORRELATION ENERGY"] = mobj.group(15)
        psivar["MP2-F12 SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(14)) * Decimal(2) / Decimal(3)
        psivar["MP2-F12 OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(15)) - psivar["MP2-F12 SAME-SPIN CORRELATION ENERGY"]
        )
    ...
    mobj = re.search(
        r"^\s*"
        + r"CCSD triplet pair energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"CCSD correlation energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"Triples \(T\) contribution\s+"
        + NUMBER
        + r"\s*$",
        outtext,
        re.MULTILINE,
    ...
    if mobj:
        ...
        psivar["CCSD SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(1)) * Decimal(2) / Decimal(3)
        psivar["CCSD OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(2)) - psivar["CCSD SAME-SPIN CORRELATION ENERGY"]
        )
    ...
    mobj = re.search(
        r"^\s*"
        + r"CCSD-F12a triplet pair energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"CCSD-F12a correlation energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"Triples \(T\) contribution\s+"
    ...
    if mobj:
        ...
        psivar["CCSD-F12A SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(1)) * Decimal(2) / Decimal(3)
        psivar["CCSD-F12A OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(2)) - psivar["CCSD-F12A SAME-SPIN CORRELATION ENERGY"]
        )
        ...
        psivar["CCSD(T)-F12A TOTAL ENERGY"] = (
            psivar["CCSD(T)-F12A CORRELATION ENERGY"] + psivar["HF-CABS TOTAL ENERGY"]
        )
        psivar["(T*)-F12AB CORRECTION ENERGY"] = (
            Decimal(mobj.group(3)) * psivar["MP2-F12 CORRELATION ENERGY"] / psivar["MP2 CORRELATION ENERGY"]
        )
    ...
    mobj = re.search(
        r"^\s*"
        + r"CCSD-F12b triplet pair energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"CCSD-F12b correlation energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"Triples \(T\) contribution\s+"
    ...
    if mobj:
        ...
        psivar["CCSD-F12B SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(1)) * Decimal(2) / Decimal(3)
        psivar["CCSD-F12B OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(2)) - psivar["CCSD-F12B SAME-SPIN CORRELATION ENERGY"]
        )
        ...
        psivar["CCSD(T)-F12B TOTAL ENERGY"] = (
            psivar["CCSD(T)-F12B CORRELATION ENERGY"] + psivar["HF-CABS TOTAL ENERGY"]
        )
        psivar["(T*)-F12AB CORRECTION ENERGY"] = (
            Decimal(mobj.group(3)) * psivar["MP2-F12 CORRELATION ENERGY"] / psivar["MP2 CORRELATION ENERGY"]
        )
    ...
    mobj = re.search(
        r"^\s*"
        + r"CCSD-F12c triplet pair energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"CCSD-F12c correlation energy\s+"
        + NUMBER
        + r"\s*"
        + r"^\s*"
        + r"Triples \(T\) contribution\s+"
    ...
    if mobj:
        ...
        psivar["CCSD-F12C SAME-SPIN CORRELATION ENERGY"] = Decimal(mobj.group(1)) * Decimal(2) / Decimal(3)
        psivar["CCSD-F12C OPPOSITE-SPIN CORRELATION ENERGY"] = (
            Decimal(mobj.group(2)) - psivar["CCSD-F12C SAME-SPIN CORRELATION ENERGY"]
        )
        ...
        psivar["CCSD(T)-F12C TOTAL ENERGY"] = (
            psivar["CCSD(T)-F12C CORRELATION ENERGY"] + psivar["HF-CABS TOTAL ENERGY"]
        )
        psivar["(T*)-F12C CORRECTION ENERGY"] = (
            Decimal(mobj.group(3)) * psivar["MP2-F12 CORRELATION ENERGY"] / psivar["MP2 CORRELATION ENERGY"]
        )
...
def muster_modelchem(name, dertype, mol):
    ...
    if lowername == "mp2":
        ...
        options["DF-KS,"]["OPTIONS"]["value"] = (
            """lhf,df_basis=dflhf,basis_coul=jkfitb,basis_exch=jkfitb; dftfac,1.0; start,1101.2; save,2101.2"""
        )
        ...
        options["DF-KS"]["OPTIONS"]["value"] = (
            """,lhf,df_basis=dflhf,basis_coul=jkfitb,basis_exch=jkfitb; dftfac,1.0; start,1102.2; save,2102.2"""
        )

        proc.append(mol.format_molecule_for_molpro())
        proc.append("sapt; intermol")
        options["SAPT; INTERMOL"]["OPTIONS"]["value"] = (
            """,saptlevel=3,ca=2101.2,cb=2102.2,icpks=0,fitlevel=3,nlexfac=0.0,cfac=0.0; dfit,basis_coul=jkfit,basis_exch=jkfit,cfit_scf=3"""
        )
