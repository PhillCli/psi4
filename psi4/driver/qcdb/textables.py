...
def table_generic(
    ...
    def table_footer():
        ...
        fnmatch = re.compile(
            r"""(?P<cellpre>.*)""" + r"""(\\footnotemark)\{(?P<fntext>.*)\}""" + r"""(?P<cellpost>.*)"""
        )
    ...
    def matelem(dict_row, dict_col):
        ...
        if "misscnt" in errpiece and errpiece["misscnt"] != 0 and "tgtcnt" in errpiece:
            kw["footnote"] = r"""\footnotemark{Missing %d of %d reactions.}""" % (
                errpiece["misscnt"],
                errpiece["tgtcnt"],
            )
        else:
    ...
    for key in ["dbse", "sset", "mtd", "opt", "bas", "err"]:
        if len(locals()[key]) > 1:
            if key not in rowplan and key not in keysincolumnplan:
                print(
                    """Warning: non-first values in argument '{0}' won't """
    ...
    head1 = (
        " & ".join([r"""\multicolumn{%d}{c}{\textbf{%s}}""" % (repeat, label) for (label, repeat) in h1]) + r""" \\ """
    )
    h2 = [(k, len(list(g))) for k, g in itertools.groupby([col[2] for col in columnplan])]
    head2 = (
        " & ".join([r"""\multicolumn{%d}{c}{\textbf{%s}}""" % (repeat, label) for (label, repeat) in h2]) + r""" \\ """
    )
            ...
            for hier1 in locals()[rowplan[1]]:
                ...
                if nH > 2:
                    ...
                    for hier2 in locals()[rowplan[2]]:
                        ...
                        text.append(
                            r"""\enspace\enspace"""
                            + " & ".join([col[3](matelem(kw, col[4])) for col in columnplan])
                            + r""" \\ """
                        )
                else:
                    text.append(
                        r"""\enspace""" + " & ".join([col[3](matelem(kw, col[4])) for col in columnplan]) + r""" \\ """
                    )
            if not subjoin:
                table_footer()
        else:
