...
def compare_fcidumps(expected: str, computed: str, label: str):
    ...
    pass_1el = compare_values(
        ref_energies["ONE-ELECTRON ENERGY"], energies["ONE-ELECTRON ENERGY"], 7, label + ". 1-electron energy"
    )
    pass_2el = compare_values(
        ref_energies["TWO-ELECTRON ENERGY"], energies["TWO-ELECTRON ENERGY"], 7, label + ". 2-electron energy"
    )
    pass_scf = compare_values(
        ref_energies["SCF TOTAL ENERGY"], energies["SCF TOTAL ENERGY"], 10, label + ". SCF total energy"
    )
    pass_mp2 = compare_values(
        ref_energies["MP2 CORRELATION ENERGY"],
        energies["MP2 CORRELATION ENERGY"],
        10,
...
def energies_from_fcidump(intdump) -> Dict[str, float]:
    ...
    # Compute SCF energy
    energies["ONE-ELECTRON ENERGY"], energies["TWO-ELECTRON ENERGY"] = _scf_energy(
        Hcore, eri, np.where(epsilon < 0)[0], intdump["uhf"]
    )
    # yapf: disable
...
def _mp2_energy(ERI, epsilon, unrestricted):
    ...
    if unrestricted:
        ...
        mp2_e = np.einsum("iajb,iajb,iajb->", MO, MO, denom) + np.einsum(
            "iajb,iajb,iajb->", MO - MO.swapaxes(1, 3), MO, denom
        )
    return mp2_e
