...
def negotiate_convergence_criterion(
    dermode: Union[Tuple[str, str], Tuple[int, int]], method: str, return_optstash: bool = False
):
    r"""
...
def upgrade_interventions(method):
    try:
        ...
        if method.__name__ == "cbs":
            raise UpgradeHelper(
                method, repr(method.__name__), 1.6, " Replace cbs or complete_basis_set function with cbs string."
            )
...
def highest_analytic_derivative_available(
    method: str, proc: Optional[Dict] = None, managed_keywords: Optional[Dict] = None
    ...
    if (
        dertype == 2
        and p4util.libint2_configuration()["eri"][2] is None
        and p4util.libint2_configuration()["eri"][0] is not None
    ):
...
def highest_analytic_properties_available(
    method: str, proc: Optional[Dict] = None, managed_keywords: Optional[Dict] = None
