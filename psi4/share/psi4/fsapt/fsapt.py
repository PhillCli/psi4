...
def compute_fsapt(dirname, links5050, completeness=0.85):
    ...
    print_fragments(
        geom, Zs["A"], Qs["A"], fragkeys["A"], frags["A"], nuclear_ws["A"], orbital_ws["A"], "%s/fragA.dat" % dirname
    )
    print_fragments(
        geom, Zs["B"], Qs["B"], fragkeys["B"], frags["B"], nuclear_ws["B"], orbital_ws["B"], "%s/fragB.dat" % dirname
    )
    ...
    if os.path.exists("%s/Extern_B.xyz" % dirname):
        ...
        if os.path.exists("%s/Extern_A.xyz" % dirname):
            frags["B"]["Extern-B"] = list(
                range(len(Zs["A"]) + len(geom_extern_A), len(Zs["A"]) + len(geom_extern_A) + len(geom_extern_B))
            )
        else:
