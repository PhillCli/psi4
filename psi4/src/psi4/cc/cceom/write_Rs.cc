    ...
    for (i = 0; i < eom_params.cs_per_irrep[C_irr]; ++i) {
        ...
        if (C_irr == eom_params.prop_sym) {
            if (i == eom_params.prop_root) {
                ...
                psio_write_entry(PSIF_CC_INFO, "States per irrep", (char *)eom_params.states_per_irrep,
                                 moinfo.nirreps * sizeof(int));
        ...
        if (!params.full_matrix) {
            ...
            psio_read_entry(PSIF_CC_HBAR, "Reference expectation value", (char *)&(expectation_val), sizeof(double));
