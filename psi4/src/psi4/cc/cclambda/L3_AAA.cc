...
void L3_AAA(double ***W1, int nirreps, int I, int Gi, int J, int Gj, int K, int Gk, dpdbuf4 *T2, dpdbuf4 *F, dpdbuf4 *E,
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gjd][0], nrows, &(T2->matrix[Gik][ik][cd]), nlinks,
                    1.0, W1[Gab][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, F->matrix[Gkd][0], nrows, &(T2->matrix[Gij][ij][cd]), nlinks,
                    1.0, W1[Gab][0], ncols);
    ...
    for (Gl = 0; Gl < nirreps; Gl++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, 1.0, T2->matrix[Gkl][kl], nrows, &(E->matrix[Gji][ji][lc]), ncols,
                    1.0, W1[Gab][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gid][0], nrows, &(T2->matrix[Gjk][kj][bd]), nlinks,
                    1.0, W2[Gca][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gjd][0], nrows, &(T2->matrix[Gik][ik][bd]), nlinks,
                    1.0, W2[Gca][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, F->matrix[Gkd][0], nrows, &(T2->matrix[Gij][ij][bd]), nlinks,
                    1.0, W2[Gca][0], ncols);
    ...
    for (Gl = 0; Gl < nirreps; Gl++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, -1.0, T2->matrix[Gil][il], nrows, &(E->matrix[Gjk][jk][lb]), ncols,
                    1.0, W2[Gca][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, 1.0, T2->matrix[Gjl][jl], nrows, &(E->matrix[Gik][ik][lb]), ncols,
                    1.0, W2[Gca][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, 1.0, T2->matrix[Gkl][kl], nrows, &(E->matrix[Gji][ji][lb]), ncols,
                    1.0, W2[Gca][0], ncols);
    }

    global_dpd_->sort_3d(W2, W1, nirreps, Gijk, F->params->coltot, F->params->colidx, F->params->colorb,
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, -1.0, F->matrix[Gid][0], nrows, &(T2->matrix[Gkj][kj][ad]), nlinks,
                    1.0, W2[Gcb][0], ncols);
    ...
    for (Gl = 0; Gl < nirreps; Gl++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, 1.0, T2->matrix[Gil][il], nrows, &(E->matrix[Gjk][jk][la]), ncols,
                    1.0, W2[Gcb][0], ncols);
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 'n', nrows, ncols, nlinks, -1.0, T2->matrix[Gjl][jl], nrows, &(E->matrix[Gik][ik][la]), ncols,
                    1.0, W2[Gcb][0], ncols);
