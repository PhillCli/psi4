...
void td_print() {
    ...
    for (i = 0; i < params.nstates; i++) {
        ...
        outfile->Printf("\t %d%3s %7.3lf %9.1lf %7.1lf %10.6lf %8.4lf %8.4lf %8.4lf  %7.6E\n", td_params[i].root + 1,
                        moinfo.labels[td_params[i].irrep].c_str(), td_params[i].cceom_energy * pc_hartree2ev,
                        td_params[i].cceom_energy * pc_hartree2wavenumbers,
                        1 / (td_params[i].cceom_energy * _hartree2nm), td_params[i].cceom_energy, td_params[i].OS,
                        td_params[i].RS_length, td_params[i].RS_velocity, td_params[i].einstein_a);
    }
    outfile->Printf("\n");
}
}  // namespace ccdensity
