...
void DFOCC::occ_iterations() {
    ...
    if (do_diis_ == 1) {
        ...
        orbitalDIIS = std::make_shared<DIISManager>(cc_maxdiis_, "Orbital Optimized DIIS",
                                                    DIISManager::RemovalPolicy::LargestError,
                                                    DIISManager::StoragePolicy::OnDisk);  // initialize DIIS manager
        ...
        if (reference_ == "RESTRICTED") {
            if (wfn_type_ == "DF-OMP2" || wfn_type_ == "DF-OLCCD" || wfn_type_ == "DF-OREMP") {
                ...
                auto T2 = std::make_shared<Matrix>("T2", naoccA * navirA,
                                                   naoccA * navirA);  // T2_1 and T2_2 share the same prototype
            ...
            if (wfn_type_ == "DF-OMP2") {
                ...
                auto RT2AA = std::make_shared<Matrix>(
                    "RT2AA", naoccA * naoccA,
                    navirA * navirA);  // the amplitudes and the residuums differ in their signature
                auto RT2BB = std::make_shared<Matrix>(
                    "RT2BB", naoccB * naoccB, navirB * navirB);  // a possible solution would be to reorder the residuum
                auto RT2AB = std::make_shared<Matrix>(
                    "RT2AB", naoccA * naoccB, navirA * navirB);  // but this would be computationally wasted time
                orbitalDIIS->set_error_vector_size(kappa_barA_.get(), kappa_barB_.get(), RT2AA.get(), RT2BB.get(),
                                                   RT2AB.get());
                ...
                auto T2AA = std::make_shared<Matrix>(
                    "T2AA", naoccA * naoccA,
                    navirA * navirA);  // T2_1 and T2_2 and their respective residuals share the same prototype
                ...
                orbitalDIIS->set_error_vector_size(kappa_barA_.get(), kappa_barB_.get(), T2AA.get(), T2BB.get(),
                                                   T2AB.get(), T2AA.get(), T2BB.get(), T2AB.get());
                orbitalDIIS->set_vector_size(kappa_barA_.get(), kappa_barB_.get(), T2AA.get(), T2BB.get(), T2AB.get(),
    ...
    do {
        ...
        if (rms_wog >= DIVERGE) {
            throw PSIEXCEPTION("DF-OCC iterations are diverging");
             ...
             std::fabs(last_rms_wog - rms_wog) >= 10 * tol_Eod);
...
void DFOCC::oo_diis() {
    ...
    if (reference_ == "RESTRICTED") {
        if (wfn_type_ == "DF-OMP2") {
            ...
            if (orbitalDIIS->subspace_size() >= cc_mindiis_) {
                ...
                for (int i = 0; i < nidpA; i++) {
                    kappa_barA->set(i, kappa_barA_vec->get(i));
                }
            }
        ...
        if (wfn_type_ == "DF-OMP2") {
            ...
            // all complete. now call add_entry, first pass the error vector componenets, then the guess vector
            // components in the same order as defined by set_error_vector_size and set_vector_size
            ...
            if (orbitalDIIS->subspace_size() >= cc_mindiis_) {
                ...
                orbitalDIIS->extrapolate(kappa_barA_vec.get(), kappa_barB_vec.get(), T1AA.get(), T1BB.get(),
                                         T1AB.get());
                ...
                for (int i = 0; i < nidpA; i++) {
                    kappa_barA->set(i, kappa_barA_vec->get(i));
                }  // please don't tell me that this is the only way to get the rotations back to kappa_barA...
                for (int i = 0; i < nidpB; i++) {
                    kappa_barB->set(i, kappa_barB_vec->get(i));
                }  // if someone can do better: go for it
            ...
            orbitalDIIS->add_entry(wogA_vec.get(), wogB_vec.get(), RT1AA.get(), RT1BB.get(), RT1AB.get(), RT2AA.get(),
                                   RT2BB.get(), RT2AB.get(), kappa_barA_vec.get(), kappa_barB_vec.get(), T1AA.get(),
                                   T1BB.get(), T1AB.get(), T2AA.get(), T2BB.get(), T2AB.get());
            if (orbitalDIIS->subspace_size() >= cc_mindiis_) {
