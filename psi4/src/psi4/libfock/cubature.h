...
class MolecularGrid {
   ...
   public:
    ...
    /// Set of spatially sieved blocks of points of a given atom, generated by sieve() internally. Only for
    /// blockscheme=atomic!
...
class DFTGrid : public MolecularGrid {
   protected:
    ...
    std::shared_ptr<BasisSet> primary_;
    /// Master builder methods
    void buildGridFromOptions(std::map<std::string, int> int_opts_map, std::map<std::string, std::string> str_opts_map,
