...
void DCTSolver::half_transform(dpdbuf4 *SO, dpdbuf4 *MO, SharedMatrix &C1, SharedMatrix &C2, int *mospi_left,
            ...
            if (mospi_left[Gc] && mospi_right[Gd] && nsopi_[Gc] && nsopi_[Gd]) {
                if (backwards) {
                    ...
                    for (ij = 0; ij < MO->params->rowtot[h]; ij++) {
                        C_DGEMM('n', 't', mospi_left[Gc], nsopi_[Gd], mospi_right[Gd], 1.0, &(MO->matrix[h][ij][cd]),
                                mospi_right[Gd], &(pC2[0][0]), mospi_right[Gd], 0.0, &(X[0][0]), nsopi_[Gd]);

                        C_DGEMM('n', 'n', nsopi_[Gc], nsopi_[Gd], mospi_left[Gc], alpha, &(pC1[0][0]), mospi_left[Gc],
                                &(X[0][0]), nsopi_[Gd], beta, &(SO->matrix[h][ij][pq]), nsopi_[Gd]);
                    }
                } else {
                    X = block_matrix(nsopi_[Gc], mospi_right[Gd]);

                    for (ij = 0; ij < MO->params->rowtot[h]; ij++) {
                        C_DGEMM('n', 'n', nsopi_[Gc], mospi_right[Gd], nsopi_[Gd], 1.0, &(SO->matrix[h][ij][pq]),
                                nsopi_[Gd], &(pC2[0][0]), mospi_right[Gd], 0.0, &(X[0][0]), mospi_right[Gd]);

                        C_DGEMM('t', 'n', mospi_left[Gc], mospi_right[Gd], nsopi_[Gc], alpha, &(pC1[0][0]),
                                mospi_left[Gc], &(X[0][0]), mospi_right[Gd], beta, &(MO->matrix[h][ij][cd]),
                                mospi_right[Gd]);
                    }
                }
    ...
    dct_timer_off("DCTSolver::half_transform");
}
...
void DCTSolver::file2_transform(dpdfile2 *SO, dpdfile2 *MO, SharedMatrix C, bool backwards) {
    ...
    dct_timer_off("DCTSolver::file2_transform");
}
