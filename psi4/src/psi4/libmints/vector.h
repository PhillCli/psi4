...
class IrreppedVector {
   ...
   public:
    ...
    T get(int m) const {
        if (m >= dimpi_.sum()) {
            throw PSIEXCEPTION("Cannot get element " + std::to_string(m) + " since there are only " +
                               std::to_string(dimpi_.sum()) + " elements.");
        }
        return v_[m];
    }
    T get(int h, int m) const {
        if (h >= nirrep()) {
            throw PSIEXCEPTION("Cannot get an element of irrep " + std::to_string(h) + ", since there are only " +
                               std::to_string(nirrep()) + " irreps.");
        }
        if (m >= dim(h)) {
            throw PSIEXCEPTION("Cannot get element " + std::to_string(m) + " of irrep " + std::to_string(h) +
                               " which only has " + std::to_string(dim(h)) + " elements.");
        }
        return vector_[h][m];
    }
    ...
    void set(int m, T val) {
        if (m >= dimpi_.sum()) {
            throw PSIEXCEPTION("Cannot set element " + std::to_string(m) + " since there are only " +
                               std::to_string(dimpi_.sum()) + " elements.");
        }
        v_[m] = val;
    }
    void set(int h, int m, T val) {
        if (h >= nirrep()) {
            throw PSIEXCEPTION("Cannot set an element of irrep " + std::to_string(h) + ", since there are only " +
                               std::to_string(nirrep()) + " irreps.");
        }
        if (m >= dim(h)) {
            throw PSIEXCEPTION("Cannot set element " + std::to_string(m) + " of irrep " + std::to_string(h) +
                               " which only has " + std::to_string(dim(h)) + " elements.");
        }
        vector_[h][m] = val;
    }
    ...
    void add(int m, T val) {
        if (m >= dimpi_.sum()) {
            throw PSIEXCEPTION("Cannot add to element " + std::to_string(m) + " since there are only " +
                               std::to_string(dimpi_.sum()) + " elements.");
        }
        v_[m] += val;
    }
    void add(int h, int m, T val) {
        if (h >= nirrep()) {
            throw PSIEXCEPTION("Cannot add to an element of irrep " + std::to_string(h) + ", since there are only " +
                               std::to_string(nirrep()) + " irreps.");
        }
        if (m >= dim(h)) {
            throw PSIEXCEPTION("Cannot add to element " + std::to_string(m) + " of irrep " + std::to_string(h) +
                               " which only has " + std::to_string(dim(h)) + " elements.");
        }
        vector_[h][m] += val;
    }
    ...
    void sort(std::function<bool(int, T, T)> func) { sort(func, Slice(Dimension(nirrep()), dimpi_)); }
    void sort(std::function<bool(int, T, T)> func, Slice slice) {
        for (int h = 0; h < nirrep(); h++) {
            std::stable_sort(vector_[h] + slice.begin()[h], vector_[h] + slice.end()[h],
                             std::bind(func, h, std::placeholders::_1, std::placeholders::_2));
        }
    }
    ...
    void set_block(const Slice &slice, const IrreppedVector<T> &block) {
        ...
        for (int h = 0; h < nirrep(); h++) {
            if (slice.end()[h] > dimpi_[h]) {
                std::string msg = "Invalid call to get_block(): Slice is out of bounds. Irrep = " + std::to_string(h);
                throw PSIEXCEPTION(msg);
            }
        }
        const Dimension &slice_begin = slice.begin();
