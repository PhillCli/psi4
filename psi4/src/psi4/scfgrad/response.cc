...
#ifdef USING_BrianQC
            if (brianEnable) {
                if (segmentAtomCount < 0 || A < segmentAtomIndexStart ||
                    A >= (segmentAtomIndexStart + segmentAtomCount)) {
                    segmentAtomIndexStart = A;
                    segmentAtomCount = (segmentAtomIndexStart + maxSegmentAtomCount > natom)
                                           ? (natom - segmentAtomIndexStart)
                                           : maxSegmentAtomCount;

                    brianInt integralType = BRIAN_INTEGRAL_TYPE_OVERLAP;
                    brianCPHFBuild1eDeriv(&brianCookie, &integralType, &segmentAtomCount, &segmentAtomIndexStart,
                                          Smn.data());
                }

                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Smnx[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Smix->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Smny[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Smiy->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Smnz[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Smiz->get_pointer(), nocc);
            } else {
#endif
                Smix->zero();
                Smiy->zero();
    ...
    // => Tpi <= //
    {
        ...
        for (int A = 0; A < natom; A++) {
            ...
            if (brianEnable) {
                ...
                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Tmnx[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Tmix->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Tmny[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Tmiy->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 2.0, Tmnz[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Tmiz->get_pointer(), nocc);
            } else {
#endif
                Tmix->zero();
                Tmiy->zero();
    ...
    if (basisset_->has_ECP()) {
...
#ifdef USING_ecpint
        {
            ...
            for (int A = 0; A < natom; A++) {
                ...
                psio_->write(PSIF_HESS, "Epi^A", (char*)Epip[0], static_cast<size_t>(nmo) * nocc * sizeof(double),
                             next_Epi, &next_Epi);
                // Epi_y
    ...
    // => Vpi <= //
    {
        ...
        for (int A = 0; A < natom; A++) {
            ...
            if (brianEnable) {
                ...
                C_DGEMM('N', 'N', nso, nocc, nso, 1.0, Vmnx[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Vmix->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 1.0, Vmny[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Vmiy->get_pointer(), nocc);
                C_DGEMM('N', 'N', nso, nocc, nso, 1.0, Vmnz[A - segmentAtomIndexStart]->get_pointer(), nso,
                        Cocc->get_pointer(), nocc, 0.0, Vmiz->get_pointer(), nocc);
            } else {
#endif
                Vmix->zero();
                Vmiy->zero();
...
#pragma omp parallel for
            for (int p = 0; p < np; p++) {
                C_DGEMM('T', 'N', nocc, nocc, nso, 1.0, Amip[p], nocc, Cop[0], nocc, 0.0,
                        &Aijp[0][p * (size_t)nocc * nocc], nocc);
            }
            ...
            if (brianEnable) {
                ...
                for (int A = 0; A < natom; A++) {
                    if (segmentAtomCount < 0 || A < segmentAtomIndexStart ||
                        ...
                        brianInt integralType = BRIAN_INTEGRAL_TYPE_NUCLEAR;
                        brianCPHFBuildRepulsionDeriv(&brianCookie, &computeCoulomb, &computeExchange, &segmentAtomCount,
                                                     &segmentAtomIndexStart, Dt->get_pointer(), nullptr, Jmn.data(),
                                                     Kmn.data(), nullptr);
                    }
...
#pragma omp parallel for schedule(dynamic) num_threads(nthreads)
                    ...
                    for (int a = 0; a < nA; ++a) {
                        ...
                        Gpi->transform(C, G, Cocc);
                        Gpi->scale(0.5);
                        psio_->write(PSIF_HESS, "Gpi^A", (char*)pGpi[0],
                                     static_cast<size_t>(nmo) * nocc * sizeof(double), next_Gpi, &next_Gpi);
                    }
                }  // End loop over A batches
#ifdef USING_BrianQC
            }
    ...
    // => Fpi <= //
    {
        ...
        for (int A = 0; A < 3 * natom; A++) {
            ...
            Fpi->add(Tpi);
            if (basisset_->has_ECP()) {
                psio_->read(PSIF_HESS, "Epi^A", (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double),
                            next_Epi, &next_Epi);
                Fpi->add(Tpi);
            }
...
std::shared_ptr<Matrix> USCFDeriv::hessian_response() {
    ...
    // so we'll leave it here rather than in a separate function
    {
        ...
        for (int A = 0; A < 3 * natom; A += max_A) {
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->read(PSIF_HESS, "Bai^A_a", (char*)Tap[0], static_cast<size_t>(navir) * naocc * sizeof(double),
                            next_Baia, &next_Baia);
                ...
                psio_->read(PSIF_HESS, "Bai^A_b", (char*)Tbp[0], static_cast<size_t>(nbvir) * nbocc * sizeof(double),
                            next_Baib, &next_Baib);
            ...
            for (int a = 0; a < nA; a++) {
                ...
                psio_->write(PSIF_HESS, "Uai^A_a", (char*)Tap[0], static_cast<size_t>(navir) * naocc * sizeof(double),
                             next_Uaia, &next_Uaia);
            }
            // Write Ubs to disk
            for (int a = 0; a < nA; a++) {
                ...
                psio_->write(PSIF_HESS, "Uai^A_b", (char*)Tbp[0], static_cast<size_t>(nbvir) * nbocc * sizeof(double),
                             next_Uaib, &next_Uaib);
            }
        }
    }
    ...
    // => Zipper <= //
    {
        ...
        for (int A = 0; A < 3 * natom; A += max_a) {
            ...
            La->add(Ta);
            for (int i = 0; i < naocc; i++) C_DSCAL(static_cast<size_t>(nA) * nmo, eao_p[i], &Lap[0][i], naocc);
            for (int B = 0; B < 3 * natom; B += max_a) {
                int nB = (B + max_a >= 3 * natom ? 3 * natom - B : max_a);
                psio_address nextB = psio_get_address(PSIO_ZERO, B * npia * sizeof(double));
        ...
        for (int A = 0; A < 3 * natom; A += max_b) {
            ...
            Lb->add(Tb);
            for (int i = 0; i < nbocc; i++) C_DSCAL(static_cast<size_t>(nA) * nmo, ebo_p[i], &Lbp[0][i], nbocc);
            for (int B = 0; B < 3 * natom; B += max_b) {
                int nB = (B + max_b >= 3 * natom ? 3 * natom - B : max_b);
                psio_address nextB = psio_get_address(PSIO_ZERO, B * npib * sizeof(double));
...
void USCFDeriv::overlap_deriv(std::shared_ptr<Matrix> C, std::shared_ptr<Matrix> Cocc, std::shared_ptr<Matrix> Cvir,
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, Smi_str, (char*)Smixp[0], static_cast<size_t>(nso) * nocc * sizeof(double), next_Smi,
                     &next_Smi);
    }
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, Sai_str, (char*)Saip[0], static_cast<size_t>(nvir) * nocc * sizeof(double), next_Sai,
                     &next_Sai);
    }
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, Sij_str, (char*)Sijp[0], static_cast<size_t>(nocc) * nocc * sizeof(double), next_Sij,
                     &next_Sij);
    }
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, Spi_str, (char*)Spip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Spi,
                     &next_Spi);
    }
    ...
    for (int A = 0; A < natom; A++) {
        ...
        psio_->write(PSIF_HESS, Smi_str, (char*)Smixp[0], static_cast<size_t>(nso) * nocc * sizeof(double), next_Smi,
                     &next_Smi);
        // Smi_y
        psio_->write(PSIF_HESS, Smi_str, (char*)Smiyp[0], static_cast<size_t>(nso) * nocc * sizeof(double), next_Smi,
                     &next_Smi);
        // Smi_z
        psio_->write(PSIF_HESS, Smi_str, (char*)Smizp[0], static_cast<size_t>(nso) * nocc * sizeof(double), next_Smi,
                     &next_Smi);
...
void USCFDeriv::kinetic_deriv(std::shared_ptr<Matrix> C, std::shared_ptr<Matrix> Cocc, int nso, int nocc, int nvir,
                              bool alpha) {
    // Kinetic derivatives
    std::shared_ptr<OneBodyAOInt> Tint(integral_->ao_kinetic(1));
    size_t nmo = nocc + nvir;
    ...
    for (int A = 0; A < natom; A++) {
        ...
        // Tpi_x
        C_DGEMM('T', 'N', nmo, nocc, nso, 0.5, Cp[0], nmo, Tmixp[0], nocc, 0.0, Tpip[0], nocc);
        psio_->write(PSIF_HESS, Tpi_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Tpi,
                     &next_Tpi);
        // Tpi_y
        C_DGEMM('T', 'N', nmo, nocc, nso, 0.5, Cp[0], nmo, Tmiyp[0], nocc, 0.0, Tpip[0], nocc);
        psio_->write(PSIF_HESS, Tpi_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Tpi,
...
#ifdef USING_ecpint
void USCFDeriv::ecp_deriv(std::shared_ptr<Matrix> C, std::shared_ptr<Matrix> Cocc, int nso, int nocc, int nvir,
                          bool alpha) {
    std::shared_ptr<ECPInt> ecpint(dynamic_cast<ECPInt*>(integral_->ao_ecp(1).release()));
    size_t nmo = nocc + nvir;
    int natom = molecule_->natom();
...
void USCFDeriv::potential_deriv(std::shared_ptr<Matrix> C, std::shared_ptr<Matrix> Cocc, int nso, int nocc, int nvir,
                                bool alpha) {
    std::shared_ptr<OneBodyAOInt> Vint(integral_->ao_potential(1));
    size_t nmo = nocc + nvir;
    int natom = molecule_->natom();
    ...
    for (int A = 0; A < natom; A++) {
        ...
        // Vpi_x
        C_DGEMM('T', 'N', nmo, nocc, nso, 1.0, Cp[0], nmo, Vmixp[0], nocc, 0.0, Vpip[0], nocc);
        psio_->write(PSIF_HESS, Vpi_str, (char*)Vpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Vpi,
                     &next_Vpi);
        // Vpi_y
        C_DGEMM('T', 'N', nmo, nocc, nso, 1.0, Cp[0], nmo, Vmiyp[0], nocc, 0.0, Vpip[0], nocc);
        psio_->write(PSIF_HESS, Vpi_str, (char*)Vpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Vpi,
...
void USCFDeriv::JK_deriv1(std::shared_ptr<Matrix> D1, std::shared_ptr<Matrix> C1, std::shared_ptr<Matrix> C1occ,
                          std::shared_ptr<Matrix> D2, int nso, int nocc, int nvir, bool alpha) {
    ...
    std::vector<double**> pdG(3 * natom);
    std::vector<bool> pert_incore(3 * natom);
    for (int a = 0; a < max_a; ++a) dGmats.push_back(std::make_shared<Matrix>("G derivative contribution", nso, nso));
    ...
    // Write some junk for now, to set the sizing for PSIO
    for (int pert = 0; pert < 3 * natom; ++pert) {
        psio_->write(PSIF_HESS, Gpi_str, (char*)pGpi[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Gpi,
                     &next_Gpi);
    }
...
#pragma omp parallel for
        ...
        for (int A = 0; A < 3 * natom; A += max_a) {
                    ...
                    for (int N = 0; N < nshell; ++N) {
                        ...
                        if (pert_incore[Pcenter]) {
                            ...
                            if (Kscale) {
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&PxBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[Px][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&PyBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[Py][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&PzBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[Pz][oN], nso);
                            }
                        }
                        if (pert_incore[Mcenter]) {
                            ...
                            if (Kscale) {
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&mxBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[mx][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&myBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[my][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&mzBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[mz][oN], nso);
                            }
                        }
                        if (pert_incore[Ncenter]) {
                            ...
                            if (Kscale) {
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&nxBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[nx][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&nyBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[ny][oN], nso);
                                for (int p = 0; p < nP; ++p)
                                    C_DGEMM('T', 'N', nN, nso, nM, -2 * Kscale,
                                            const_cast<double*>(&nzBuf[p * nM * nN]), nN, &pTmn[oP + p][oM * nso], nso,
                                            1.0, pdG[nz][oN], nso);
                            }
                        }
                    }
            ...
            for (int a = 0; a < nA; ++a) {
                ...
                Gpi->scale(0.5);
                psio_->write(PSIF_HESS, Gpi_str, (char*)pGpi[0], static_cast<size_t>(nmo) * nocc * sizeof(double),
                             next_Gpi, &next_Gpi);
            }
        }  // End loop over A batches
        ...
        if (brianEnable) {
            ...
            for (int A = 0; A < natom; A++) {
                if (segmentAtomCount < 0 || A < segmentAtomIndexStart ||
                    ...
                    brianInt integralType = BRIAN_INTEGRAL_TYPE_NUCLEAR;
                    brianCPHFBuildRepulsionDeriv(&brianCookie, &computeCoulomb, &computeExchange, &segmentAtomCount,
                                                 &segmentAtomIndexStart, D1->get_pointer(), D2->get_pointer(),
                                                 Jmn.data(), Kmn1.data(), Kmn2.data());
                }
...
void USCFDeriv::JK_deriv2(std::shared_ptr<JK> jk, int mem, std::shared_ptr<Matrix> Ca, std::shared_ptr<Matrix> Caocc,
                          std::shared_ptr<Matrix> Cb, std::shared_ptr<Matrix> Cbocc, int nso, int naocc, int nbocc,
                          int navir) {
    // => J2pi/K2pi <= //
    auto Cap = Ca->pointer();
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, "G2pi^A_a", (char*)Uap[0], static_cast<size_t>(nmo) * naocc * sizeof(double), next_GpiA,
                     &next_GpiA);
    }
    ...
    for (int A = 0; A < 3 * natom; A += max_A) {
        ...
        for (int a = 0; a < nA; a++) {
            ...
            psio_->read(PSIF_HESS, "Sij^A_a", (char*)Sijap[0], static_cast<size_t>(naocc) * naocc * sizeof(double),
                        next_Sij, &next_Sij);
            C_DGEMM('N', 'N', nso, naocc, naocc, 1.0, Caop[0], naocc, Sijap[0], naocc, 0.0, R[2 * a]->pointer()[0],
                    naocc);
        ...
        for (int a = 0; a < nA; a++) {
            ...
            psio_->read(PSIF_HESS, "Sij^A_b", (char*)Sijbp[0], static_cast<size_t>(nbocc) * nbocc * sizeof(double),
                        next_Sij, &next_Sij);
            C_DGEMM('N', 'N', nso, nbocc, nbocc, 1.0, Cbop[0], nbocc, Sijbp[0], nbocc, 0.0, R[2 * a + 1]->pointer()[0],
                    nbocc);
        ...
        for (int a = 0; a < nA; a++) {
            ...
            C_DGEMM('N', 'N', nso, nbocc, nso, 1.0, J[2 * a + 1]->pointer()[0], nso, Cbop[0], nbocc, 0.0, Tbp[0],
                    nbocc);
            ...
            psio_->write(PSIF_HESS, "G2pi^A_a", (char*)Uap[0], static_cast<size_t>(nmo) * naocc * sizeof(double),
                         next_GpiA, &next_GpiA);

            psio_address next_GpiB = psio_get_address(PSIO_ZERO, (A + a) * (size_t)nmo * nbocc * sizeof(double));
            psio_->write(PSIF_HESS, "G2pi^A_b", (char*)Ubp[0], static_cast<size_t>(nmo) * nbocc * sizeof(double),
                         next_GpiB, &next_GpiB);
        }
    }
}

void USCFDeriv::VXC_deriv(std::shared_ptr<Matrix> Ca, std::shared_ptr<Matrix> Caocc, std::shared_ptr<Matrix> Cb,
                          std::shared_ptr<Matrix> Cbocc, int nso, int naocc, int nbocc, int navir) {
    // => XC Gradient <= //
    auto Cap = Ca->pointer();
    auto Caop = Caocc->pointer();
    auto Cbp = Cb->pointer();
    ...
    if (functional_->needs_xc()) {
        ...
        for (int A = 0; A < 3 * natom; A++)
            psio_->write(PSIF_HESS, "VXCpi^A_a", (char*)Uap[0], static_cast<size_t>(nmo) * naocc * sizeof(double),
                         next_VXCpi, &next_VXCpi);
        ...
        for (int a = 0; a < 3 * natom; ++a) {
            // Transform from SO basis to pi
            C_DGEMM('N', 'N', nso, naocc, nso, 1.0, Vxc_matrices[2 * a]->pointer()[0], nso, Caop[0], naocc, 0.0, Tap[0],
                    naocc);
            C_DGEMM('T', 'N', nmo, naocc, nso, 1.0, Cap[0], nmo, Tap[0], naocc, 0.0, Uap[0], naocc);
        ...
        for (int a = 0; a < 3 * natom; ++a) {
            // Transform from SO basis to pi
            C_DGEMM('N', 'N', nso, nbocc, nso, 1.0, Vxc_matrices[2 * a + 1]->pointer()[0], nso, Cbop[0], nbocc, 0.0,
                    Tbp[0], nbocc);
            C_DGEMM('T', 'N', nmo, nbocc, nso, 1.0, Cbp[0], nmo, Tbp[0], nbocc, 0.0, Ubp[0], nbocc);
            next_VXCpi = psio_get_address(PSIO_ZERO, a * (size_t)nmo * nbocc * sizeof(double));

            psio_->write(PSIF_HESS, "VXCpi^A_b", (char*)Ubp[0], static_cast<size_t>(nmo) * nbocc * sizeof(double),
                         next_VXCpi, &next_VXCpi);
        }
    }
}
...
void USCFDeriv::assemble_Fock(int nocc, int nvir, bool alpha) {
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->read(PSIF_HESS, T_str, (char*)Fpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Tpi,
                    &next_Tpi);
        psio_->read(PSIF_HESS, V_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Vpi,
                    &next_Vpi);
        Fpi->add(Tpi);
        if (basisset_->has_ECP()) {
            psio_->read(PSIF_HESS, E_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Epi,
                        &next_Epi);
            Fpi->add(Tpi);
        }
        psio_->read(PSIF_HESS, G_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Jpi,
                    &next_Jpi);
        Fpi->add(Tpi);
        if (functional_->needs_xc()) {
            psio_->read(PSIF_HESS, VXC_str, (char*)Tpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double),
                        next_VXCpi, &next_VXCpi);
            Fpi->add(Tpi);
        }

        psio_->write(PSIF_HESS, F_str, (char*)Fpip[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Fpi,
                     &next_Fpi);
    }
}
...
void USCFDeriv::assemble_B(std::shared_ptr<Vector> eocc, int nocc, int nvir, bool alpha) {
    ...
    for (int A = 0; A < 3 * natom; A++) {
        next_Fpi = psio_get_address(PSIO_ZERO, sizeof(double) * (A * (size_t)nmo * nocc + nocc * nocc));
        psio_->read(PSIF_HESS, F_str, (char*)Baip[0], static_cast<size_t>(nvir) * nocc * sizeof(double), next_Fpi,
                    &next_Fpi);
        next_Spi = psio_get_address(PSIO_ZERO, sizeof(double) * (A * (size_t)nmo * nocc + nocc * nocc));
        psio_->read(PSIF_HESS, S_str, (char*)Taip[0], static_cast<size_t>(nvir) * nocc * sizeof(double), next_Spi,
                    &next_Spi);
        for (int i = 0; i < nocc; i++) C_DAXPY(nvir, -eop[i], &Taip[0][i], nocc, &Baip[0][i], nocc);
        next_G2pi = psio_get_address(PSIO_ZERO, sizeof(double) * (A * (size_t)nmo * nocc + nocc * nocc));
        psio_->read(PSIF_HESS, G_str, (char*)Taip[0], static_cast<size_t>(nvir) * nocc * sizeof(double), next_G2pi,
                    &next_G2pi);
        Bai->add(Tai);
        Bai->scale(-1.0);
        psio_->write(PSIF_HESS, B_str, (char*)Baip[0], static_cast<size_t>(nvir) * nocc * sizeof(double), next_Bai,
                     &next_Bai);
    }
}
...
void USCFDeriv::assemble_U(int nocc, int nvir, bool alpha) {
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->read(PSIF_HESS, S_str, (char*)Upqp[0], static_cast<size_t>(nocc) * nocc * sizeof(double), next_Spi,
                    &next_Spi);
        C_DSCAL(nocc * (size_t)nocc, -0.5, Upqp[0], 1);
        psio_->read(PSIF_HESS, Uai_str, (char*)Upqp[nocc], static_cast<size_t>(nvir) * nocc * sizeof(double), next_Uai,
                    &next_Uai);
        psio_->write(PSIF_HESS, Upi_str, (char*)Upqp[0], static_cast<size_t>(nmo) * nocc * sizeof(double), next_Upi,
                     &next_Upi);
    }
}

void USCFDeriv::assemble_Q(std::shared_ptr<JK> jk, std::shared_ptr<Matrix> Ca, std::shared_ptr<Matrix> Caocc,
                           std::shared_ptr<Matrix> Cb, std::shared_ptr<Matrix> Cbocc, int nso, int naocc, int nbocc,
                           int navir) {
    // => Qpi <= //
    size_t nmo = naocc + navir;
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->write(PSIF_HESS, "Qpi^A_a", (char*)Uap[0], static_cast<size_t>(nmo) * naocc * sizeof(double), next_QpiA,
                     &next_QpiA);
    }
    ...
    for (int A = 0; A < 3 * natom; A += max_A) {
        ...
        for (int a = 0; a < nA; a++) {
            ...
            psio_->read(PSIF_HESS, "Upi^A_a", (char*)Uapip[0], static_cast<size_t>(nmo) * naocc * sizeof(double),
                        next_Upi, &next_Upi);
        ...
        for (int a = 0; a < nA; a++) {
            ...
            psio_->read(PSIF_HESS, "Upi^A_b", (char*)Ubpip[0], static_cast<size_t>(nmo) * nbocc * sizeof(double),
                        next_Upi, &next_Upi);
            C_DGEMM('N', 'N', nso, nbocc, nmo, 1.0, Cbp[0], nmo, Ubpip[0], nbocc, 0.0, R[2 * a + 1]->pointer()[0],
                    nbocc);
            Dx[2 * a + 1] = linalg::doublet(L[2 * a + 1], R[2 * a + 1], false, true);
            Dx[2 * a + 1]->hermitivitize();
        }
        ...
        for (int a = 0; a < nA; a++) {
            ...
            psio_->write(PSIF_HESS, "Qpi^A_a", (char*)Uap[0], static_cast<size_t>(nmo) * naocc * sizeof(double),
                         next_QpiA, &next_QpiA);

            psio_address next_QpiB = psio_get_address(PSIO_ZERO, (A + a) * (size_t)nmo * nbocc * sizeof(double));
            psio_->write(PSIF_HESS, "Qpi^A_b", (char*)Ubp[0], static_cast<size_t>(nmo) * nbocc * sizeof(double),
                         next_QpiB, &next_QpiB);
        }
    }
}

void USCFDeriv::dipole_derivatives(std::shared_ptr<Matrix> C1, std::shared_ptr<Matrix> C1occ,
                                   std::shared_ptr<Matrix> C2, std::shared_ptr<Matrix> C2occ,
                                   std::shared_ptr<Matrix> Dt, int nso, int n1occ, int n2occ, int n1vir) {
    // => Dipole derivatives (for IR intensities) <= //
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->read(PSIF_HESS, "Upi^A_a", (char*)U1pip[0], static_cast<size_t>(nmo) * n1occ * sizeof(double), next_Upi,
                    &next_Upi);
    ...
    for (int A = 0; A < 3 * natom; A++) {
        psio_->read(PSIF_HESS, "Upi^A_b", (char*)U2pip[0], static_cast<size_t>(nmo) * n2occ * sizeof(double), next_Upi,
                    &next_Upi);
