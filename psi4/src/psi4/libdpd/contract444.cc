...
int DPD::contract444(dpdbuf4 *X, dpdbuf4 *Y, dpdbuf4 *Z, int target_X, int target_Y, double alpha, double beta) {
    ...
    for (Hx = 0; Hx < nirreps; Hx++) {
        ...
        if (X->params->rowtot[Hx] && X->params->coltot[Hx ^ GX]) {
            ...
            if (rows_per_bucket > X->params->rowtot[Hx]) rows_per_bucket = X->params->rowtot[Hx];
        ...
        if (!incore) {
            ...
            outfile->Printf("Contract444: h = %d, row = %d, col = %d, tot = %d\n", Hx, X->params->rowtot[Hx],
                            X->params->coltot[Hx ^ GX], X->params->rowtot[Hx] * X->params->coltot[Hx ^ GX]);
