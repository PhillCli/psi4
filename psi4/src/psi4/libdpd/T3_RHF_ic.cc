...
void DPD::T3_RHF_ic(double ***W1, int nirreps, int I, int Gi, int J, int Gj, int K, int Gk, dpdbuf4 *T2, dpdbuf4 *F,
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gid][id], nrows, &(T2->matrix[Gjk][kj][cd]), nlinks,
                    1.0, W1[Gab][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gjd][jd], nrows, &(T2->matrix[Gik][ki][cd]), nlinks,
                    1.0, W2[Gab][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gid][id], nrows, &(T2->matrix[Gjk][jk][bd]), nlinks,
                    1.0, W2[Gca][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gkd][kd], nrows, &(T2->matrix[Gij][ji][bd]), nlinks,
                    1.0, W2[Gca][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gkd][kd], nrows, &(T2->matrix[Gij][ij][ad]), nlinks,
                    1.0, W2[Gcb][0], ncols);
    }
    ...
    for (Gd = 0; Gd < nirreps; Gd++) {
        ...
        if (nrows && ncols && nlinks)
            C_DGEMM('t', 't', nrows, ncols, nlinks, 1.0, F->matrix[Gjd][jd], nrows, &(T2->matrix[Gik][ik][ad]), nlinks,
                    1.0, W2[Gcb][0], ncols);
    }
