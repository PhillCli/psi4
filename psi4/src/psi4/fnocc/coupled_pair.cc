...
CoupledPair::CoupledPair(std::shared_ptr<Wavefunction> reference_wavefunction, Options &options)
    ...
    if (cepa_level == 0)
        sprintf(cepa_type, "CEPA(0)");
    else if (cepa_level == 1)
        sprintf(cepa_type, "CEPA(1)");
    else if (cepa_level == 2)
        sprintf(cepa_type, "CEPA(2)");
    else if (cepa_level == 3)
        sprintf(cepa_type, "CEPA(3)");
    else if (cepa_level == -1)
        sprintf(cepa_type, "CISD");
    else if (cepa_level == -2)
        sprintf(cepa_type, "ACPF");
    else if (cepa_level == -3)
        sprintf(cepa_type, "AQCC");
}
...
double CoupledPair::compute_energy() {
    ...
    if (cepa_level <= 0) {
        if (options_.get_bool("NAT_ORBS")) {
            // outfile->Printf("\n");
            // outfile->Printf("\n");
            // outfile->Printf("       <<< Warning >>>  %s OPDM will have no correction for FNO
            // truncation.\n",cepa_type);
            // outfile->Printf("\n");
        } else {
            OPDM();
...
PsiReturnType CoupledPair::CEPAIterations() {
    ...
    outfile->Printf("\n");
    outfile->Printf("  Begin %s iterations\n\n", cepa_type);
    ...
    while (iter < maxiter) {
        ...
        outfile->Printf("  %5i   %i %i %15.10f %15.10f %15.10f %8d\n", iter, diis_iter - 1, replace_diis_iter, dume,
                        dume - Eold, nrm, (int)iter_stop - (int)iter_start);
        ...
        for (long int j = 0; j < o; j++) {
            ...
            for (long int a = o; a < rs; a++) {
                for (long int b = o; b < rs; b++) {
                    ...
                    energy +=
                        integrals[iajb] * (2.0 * tb[ijab] - tb[(b - o) * o * o * v + (a - o) * o * o + i * o + j]);
                }
            }
        ...
        for (int j = 0; j < o; j++) {
            ...
            for (int a = o; a < rs; a++) {
                ...
                for (int b = o; b < rs; b++) {
                    ...
                    ed += (tempv[ijab] + dijab * tb[ijab]) *
                          (2.0 * tb[ijab] - tb[(b - o) * o * o * v + (a - o) * o * o + i * o + j]);
