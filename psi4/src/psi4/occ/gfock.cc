    ...
    if (reference_ == "RESTRICTED") {
        ...
        if (wfn_type_ != "OMP2") {
            ...
            if (twopdm_abcd_type == "DIRECT") {
                ...
                global_dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[O,O]"), ID("[O,V]"), ID("[O,O]"), ID("[O,V]"), 0,
                                       "X <OO|OV>");
    ...
    //===========================================================================================
        ...
        if (wfn_type_ != "OMP2") {
            if (twopdm_abcd_type == "DIRECT") {
                ...
                global_dpd_->buf4_init(&X, PSIF_OCC_DENSITY, 0, ID("[O,o]"), ID("[V,o]"), ID("[O,o]"), ID("[V,o]"), 0,
                                       "X <Oo|Vo>");
                global_dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,o]"), ID("[V,v]"), ID("[V,o]"), ID("[V,v]"), 0,
                                       "MO Ints <Vo|Vv>");
                if (wfn_type_ == "OMP3" || wfn_type_ == "OMP2.5") {
                    global_dpd_->buf4_init(&T, PSIF_OCC_DPD, 0, ID("[O,o]"), ID("[V,v]"), ID("[O,o]"), ID("[V,v]"), 0,
                                           "T2_1 <Oo|Vv>");
        ...
        */
        ...
        if (wfn_type_ != "OMP2") {
            ...
            global_dpd_->buf4_init(&K, PSIF_LIBTRANS_DPD, 0, ID("[V,o]"), ID("[V,v]"), ID("[V,o]"), ID("[V,v]"), 0,
                                   "MO Ints <Vo|Vv>");
            global_dpd_->buf4_init(&G, PSIF_OCC_DENSITY, 0, ID("[V,o]"), ID("[O,v]"), ID("[V,o]"), ID("[O,v]"), 0,
                                   "TPDM <Vo|Ov>");
            global_dpd_->contract442(&K, &G, &GF, 2, 2, 4.0, 1.0);
            global_dpd_->buf4_close(&K);
            global_dpd_->buf4_close(&G);
