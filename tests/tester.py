    ...
    if fl.name in ["input.dat", "input.py"]:
        ...
        if cml.is_file():
            ...
            if mobj:
                if mobj.group("name") != ctest_name:
                    complaints.append(
                        f"{testdir}: mismatched directory ({ctest_name}) and ctest registration name ({mobj.group('name')}). `vi {cml}`"
                    )
        ...
        if tipy.is_file():
            ...
            if (set(marks) - set(pyplugins)) != set(pymarks):
                complaints.append(
                    f"{testdir}: mismatched marks ctest ({markstr}) and pytest ({';'.join(pymarks)}). `vi {testdir}/CMakeLists.txt {testdir}/test_input.py`"
                )
            ...
            if mobj:
                if mobj.group("name") != pytest_name:
                    complaints.append(
                        f"{testdir}: mismatched directory ({pytest_name}) and pytest registration ({mobj.group('name')}). `vi {tipy}`"
                    )
