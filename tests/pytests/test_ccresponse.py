...
def test_cc_polaroptrot():
    ...
    rotlenr_589 = (
        np.trace(wfn.variable("CCSD OPTICAL ROTATION TENSOR (LEN) @ 589NM").to_array()) * prefactor * au_589
    )  # RESULT
    ...
    rotlenr_355 = (
        np.trace(wfn.variable("CCSD OPTICAL ROTATION TENSOR (LEN) @ 355NM").to_array()) * prefactor * au_355
    )  # RESULT
    ...
    polt_589 = np.array(
        [
            [5.44184081e00, -8.30270852e-01, 9.39878004e-14],
            [-8.30270852e-01, 1.27518991e01, -6.94720506e-14],
            [9.39878004e-14, -6.94720506e-14, 8.57516214e00],
        ]
    )  # TEST
    polt_355 = np.array(
        [
            [5.57913165e00, -8.66951957e-01, 1.02575190e-13],
            [-8.66951957e-01, 1.31256109e01, -7.71806997e-14],
            [1.02575190e-13, -7.71806997e-14, 8.77928702e00],
        ]
    )  # TEST
    rottvel_0 = np.array(
        [
            [3.63702792e-01, -3.62754886e-01, -3.37227243e-14],
            [-1.16320923e-01, 1.20784891e-02, 1.76483272e-14],
            [1.90062651e-14, -2.33369227e-15, -3.92022202e-01],
        ]
    )  # TEST
    rottlen_589 = np.array(
        [
            [1.41679366e-01, -7.07140738e-02, -2.26060131e-14],
            [-2.72710108e-01, 1.28928522e-03, 1.01578936e-14],
            [3.74236371e-14, 3.88522300e-15, -1.27276912e-01],
        ]
    )  # TEST
    rottvel_589 = np.array(
        [
            [3.75624000e-01, -3.74245928e-01, -3.61489832e-14],
            [-1.39409385e-01, 1.26867931e-02, 1.94630079e-14],
            [2.33993608e-14, -1.61361648e-15, -4.01726048e-01],
        ]
    )  # TEST
    rottmvg_589 = np.array(
        [
            [1.19212077e-02, -1.14910412e-02, -2.42625886e-15],
    ...
    assert psi4.compare_values(
        polt_589, wfn.variable("CCSD DIPOLE POLARIZABILITY TENSOR @ 589NM"), 3, "CCSD polarizability tensor @ 589nm"
    )  # TEST
    assert psi4.compare_values(
        polt_355, wfn.variable("CCSD DIPOLE POLARIZABILITY TENSOR @ 355NM"), 3, "CCSD polarizability tensor @ 355nm"
    )  # TEST
    assert psi4.compare_arrays(
        rottvel_0,
        wfn.variable("CCSD OPTICAL ROTATION TENSOR (VEL) @ 0NM"),
        3,
        "CCSD optrot tensor @ 0NM in velocity guage",
    )  # TEST
    assert psi4.compare_arrays(
        rottlen_589,
        wfn.variable("CCSD OPTICAL ROTATION TENSOR (LEN) @ 589NM"),
        3,
        "CCSD optrot tensor @ 589NM in length guage",
    )  # TEST
    assert psi4.compare_arrays(
        rottvel_589,
        wfn.variable("CCSD OPTICAL ROTATION TENSOR (VEL) @ 589NM"),
        3,
        "CCSD optrot tensor @ 589NM in velocity guage",
    )  # TEST
    assert psi4.compare_arrays(
        rottmvg_589,
        wfn.variable("CCSD OPTICAL ROTATION TENSOR (MVG) @ 589NM"),
        3,
        "CCSD optrot tensor @ 589NM in modified velocity guage",
    )  # TEST
...
def test_cc_roa():
    ...
    quad0 = np.array(
        [
            [
                [-2.52229282e-14, -4.33846468e-13, 3.90176525e00],
                [-4.33846468e-13, -6.81906994e-15, -5.83659009e00],
                [3.90176525e00, -5.83659009e00, 3.18177152e-14],
            ],
            [
                [1.20834634e-13, 6.99638702e-14, -2.37995874e00],
                [6.99638702e-14, -1.17603861e-13, 6.85651590e00],
                [-2.37995874e00, 6.85651590e00, -3.99609068e-15],
            ],
            [
                [-4.55024365e00, -5.30335671e00, 1.65990108e-13],
                [-5.30335671e00, -1.35415335e00, -8.75522529e-13],
                [1.65990108e-13, -8.75522529e-13, 5.90439700e00],
            ],
        ]
    )
    ...
    assert psi4.compare_values(
        quad0, wfn.variable("CCSD QUADRUPOLE POLARIZABILITY TENSOR @ 589NM"), 3, "CCSD quadrupole tensor @ 589nm"
    )  # TEST
