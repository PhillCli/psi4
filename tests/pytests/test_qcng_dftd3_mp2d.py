...
def test_dftd3__run_dftd3__2body(inp, program, subjects, request):
    ...
    assert compare_values(expected, jrec["extras"]["qcvars"]["CURRENT ENERGY"], atol=5.0e-7)
    assert compare_values(expected, jrec["extras"]["qcvars"]["DISPERSION CORRECTION ENERGY"], atol=5.0e-7)
    if program == "dftd3":
        assert compare_values(expected, jrec["extras"]["qcvars"]["2-BODY DISPERSION CORRECTION ENERGY"], atol=1.0e-7)
    assert compare_values(
        expected, jrec["extras"]["qcvars"][inp["lbl"] + " DISPERSION CORRECTION ENERGY"], atol=5.0e-7
    )

    assert compare_values(gexpected, jrec["extras"]["qcvars"]["CURRENT GRADIENT"], atol=5.0e-7)
    assert compare_values(gexpected, jrec["extras"]["qcvars"]["DISPERSION CORRECTION GRADIENT"], atol=5.0e-7)
    if program == "dftd3":
        assert compare_values(
            gexpected, jrec["extras"]["qcvars"]["2-BODY DISPERSION CORRECTION GRADIENT"], atol=1.0e-7
        )
    assert compare_values(
        gexpected, jrec["extras"]["qcvars"][inp["lbl"] + " DISPERSION CORRECTION GRADIENT"], atol=5.0e-7
    )
