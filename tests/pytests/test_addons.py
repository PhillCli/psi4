...
def test_ipi_broker1():
    ...
    frc = [
        [0.08704801, 0.1067644, -0.11170374],
...
def test_ipi_broker2():
    ...
    ref = [
        [0.000471372941, -0.006768222864, 0.000000000000],
...
def test_mp2d():
    ...
    assert psi4.compare_values(
        expected, jrec["extras"]["qcvars"]["MP2-DMP2 DISPERSION CORRECTION ENERGY"], 7, "mp2d disp E"
    )
...
def test_dftd3():
    ...
    assert psi4.compare_values(
        ref_d3bj[1], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D3 (calling dftd3 -bj)"
    )
    ...
    psi4.energy("wb97x-d")
    assert psi4.compare_values(
        -0.000834247063, psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene wb97x-d (chg)"
    )
    ...
    psi4.energy("b3lyp-d3bj")
    assert psi4.compare_values(
        ref_pbe_d3bj[2], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D3 (calling dftd3 -bj)"
    )
    ...
    psi4.energy("b3lyp-d3")
    assert psi4.compare_values(
        ref_pbe_d3zero[2], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D3 (alias)"
    )
    psi4.set_options({"dft_dispersion_parameters": [0.75]})
    psi4.energy("b3lyp-d")
    ...
    assert psi4.compare_values(
        ref_pbe_d3zero[0], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene-Ethyne -D3 (zero)"
    )
    mA = eneyne.extract_subsets(1)
    mA.run_dftd3("b3lyp", "d3zero", {"s6": 1.0, "s8": 0.722, "sr6": 1.217, "alpha6": 14.0})
    assert psi4.compare_values(
        ref_pbe_d3zero[1], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D3 (zero)"
    )
    mB = eneyne.extract_subsets(2)
    mB.run_dftd3("b3lyp", "d3zero", {"s6": 1.0, "s8": 0.722, "sr6": 1.217, "alpha6": 14.0})
    assert psi4.compare_values(
        ref_pbe_d3zero[2], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethyne -D3 (zero)"
    )

    eneyne.run_dftd3("b3lyp", "d3bj", {"s6": 1.000, "s8": 0.7875, "a1": 0.4289, "a2": 4.4407})
    assert psi4.compare_values(
        ref_pbe_d3bj[0], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene-Ethyne -D3 (bj)"
    )
    mA = eneyne.extract_subsets(1)
    ...
    assert psi4.compare_values(
        ref_pbe_d3zero[0], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene-Ethyne -D3 (alias)"
    )
    eneyne.run_dftd3("b3lyp", "d", {"s6": 0.75})
    assert psi4.compare_values(
        ref_pbe_d2[0], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene-Ethyne -D (alias)"
    )
    eneyne.run_dftd3("b3lyp", "d2", {"s6": 0.75})
    assert psi4.compare_values(
        ref_pbe_d2[0], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene-Ethyne -D2 (alias)"
    )
...
def test_sdftd3():
    ...
    try:
        ...
        psi4.energy("b3lyp-d2")
        assert psi4.compare_values(
            ref_pbe_d2[2], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D2 (alias)"
        )
    else:
...
def test_libefp():
    ...
    assert psi4.compare_values(9.1793879214, qmefp.nuclear_repulsion_energy(), 6, "QM NRE")
    assert psi4.compare_values(
        0.2622598847, psi4.variable("efp total energy") - psi4.variable("efp ind energy"), 6, "EFP corr to SCF"
    )  # from q-chem
...
def test_pcmsolver(how, request):
    ...
    assert psi4.compare_values(
        nucenergy, NH3.nuclear_repulsion_energy(), 10, "Nuclear repulsion energy (PCM, total algorithm)"
    )  # TEST
    assert psi4.compare_values(totalenergy, energy_scf1, 10, "Total energy (PCM, total algorithm)")  # TEST
    assert psi4.compare_values(
        polenergy, wfn1.variable("PCM POLARIZATION ENERGY"), 6, "Polarization energy (PCM, total algorithm)"
    )  # TEST
    ...
    assert psi4.compare_values(
        polenergy, psi4.variable("PCM POLARIZATION ENERGY"), 6, "Polarization energy (PCM, separate algorithm)"
    )
...
def test_run_json():
    ...
    json_input = {
        "schema_name": "qcschema_input",
        "schema_version": 1,
        "molecule": {"symbols": ["He", "He"], "geometry": [0, 0, -1, 0, 0, 1]},
        "driver": "gradient",
...
@uusing("cppe")
def test_cppe():
    from .test_cppe import __geoms, _dump_potential
...
def test_dftd4():
    ...
    psi4.set_options(
        {"dft_dispersion_parameters": [0.40868035, 4.53807137, 16.0, 1.0, 2.02929367, 0.0]}
    )  # b3lyp-d4-2body
    psi4.energy("b3lyp-d4(bj)")
    ...
    psi4.energy("b3lyp-d3")
    assert psi4.compare_values(
        ref_pbe_d3zero[2], psi4.variable("DISPERSION CORRECTION ENERGY"), 7, "Ethene -D4 (alias)"
    )
