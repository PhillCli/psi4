...
def runner_asserter(inp, subject, method, basis, tnm):
    qcprog = "psi4"
    qc_module_in = "-".join([qcprog, inp["keywords"].get("qc_module", "")]).strip(
        "-"
    )  # returns "psi4"|"psi4-<module>"  # input-specified routing
    qc_module_xptd = (
    ...
    if (
        qc_module_in == "psi4-ccenergy"
        and basis != "cc-pvdz"
        and method == "ccsd(t)"
        and reference == "uhf"
        and driver == "gradient"
        and inp["keywords"]["function_kwargs"]["dertype"] == 1
    ):
    ...
    using_fd = (
        driver == "gradient"
        and inp["keywords"]["function_kwargs"]["dertype"] == 0
        or driver == "hessian"
        and inp["keywords"]["function_kwargs"]["dertype"] in [0, 1]
    )  # T/F: notate fd vs. analytic for docs table
    defaultness = (
        "defaultdefault"
        if ("default" in tnm and all([not k.endswith("_type") for k in inp["keywords"]]))
        else ("default" if "default" in tnm else "")
    )
    ...
    assert compare_values(
        ref_block[f"{method.upper()} TOTAL ENERGY"], wfn.energy(), tnm + " wfn", atol=atol_e, rtol=rtol_e
    ), errmsg

    if driver == "energy":
        assert compare_values(
            ref_block[f"{method.upper()} TOTAL ENERGY"], ret, tnm + " return", atol=atol_e, rtol=rtol_e
        )
        ...
        assert compare_values(
            ref_block[f"{method.upper()} TOTAL GRADIENT"], ret.np, tnm + " grad return", atol=atol_g, rtol=rtol_g
        )
        ...
        assert compare_values(
            ref_block[f"{method.upper()} TOTAL HESSIAN"], wfn.hessian().np, tnm + " hess wfn", atol=atol_h, rtol=rtol_h
        ), errmsg
        assert compare_values(
            ref_block[f"{method.upper()} TOTAL GRADIENT"],
            wfn.gradient().np,
        ...
        assert compare_values(
            ref_block[f"{method.upper()} TOTAL HESSIAN"], ret.np, tnm + " hess return", atol=atol_h, rtol=rtol_h
        )
    ...
    if "scftype" not in tnm:
        ...
        if "default" in tnm:
            ...
            _recorder(
                ...
                corl_type,
                "fd" if using_fd else "pass",
                defaultness if defaultness == "defaultdefault" else "",
            )
        ...
        for rpv, pv, present in contractual_fn(*contractual_args):
            ...
            if present:
                ...
                if isinstance(ref_block_conv[rpv], str) and ref_block_conv[rpv] == "KnownMissing":
                    ...
                    assert compare_values(
                        ref_block_conv[rpv], query_qcvar(obj, pv), label + " v. CONV", atol=atol_conv, rtol=rtol_conv
                    ), errmsg
