...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"name": "omp2", "options": {"mp2_type": "df", "max_mograd_convergence": 6}, "varname": "DF-OMP2"},
            id="df-omp2 ae",
        ),
        pytest.param(
            {
                "name": "omp2",
                "options": {"mp2_type": "df", "freeze_core": "true", "max_mograd_convergence": 6},
                "varname": "DF-OMP2",
            },
            id="df-omp2 fc",
        ),
        pytest.param(
            {"name": "omp3", "options": {"mp_type": "df", "max_mograd_convergence": 6}, "varname": "DF-OMP3"},
            id="df-omp3 ae",
        ),
        pytest.param(
            {
                "name": "omp3",
                "options": {"mp_type": "df", "freeze_core": "true", "max_mograd_convergence": 6},
                "varname": "DF-OMP3",
            },
            id="df-omp3 fc",
        ),
        pytest.param(
            {"name": "omp2.5", "options": {"mp_type": "df", "max_mograd_convergence": 6}, "varname": "DF-OMP2.5"},
            id="df-omp2.5 ae",
        ),
        pytest.param(
            {
        ...
        pytest.param(
            {"name": "olccd", "options": {"cc_type": "df", "max_mograd_convergence": 6}, "varname": "DF-OLCCD"},
            id="df-olccd ae",
        ),
        pytest.param(
            {
                "name": "olccd",
                "options": {"cc_type": "df", "freeze_core": "true", "max_mograd_convergence": 6},
                "varname": "DF-OLCCD",
            },
            id="df-olccd fc",
        ),
        ...
        pytest.param(
            {"name": "dct", "options": {"dct_type": "df", "reference": "uhf"}, "varname": "DCT"}, id="df-udct"
        ),
        pytest.param({"name": "ccsd", "options": {"opdm_relax": "true"}, "varname": "CCSD"}, id="ccsd"),
        pytest.param(
            {"name": "ccsd", "options": {"opdm_relax": "true", "reference": "uhf"}, "varname": "CCSD"}, id="uccsd"
        ),
    ],
)
def test_dipole(inp):
    ...
    findif_dipole = [
        0,
        0,
        (8 * energies[1] - 8 * energies[-1] - energies[2] + energies[-2]) / (12 * perturbation_strength),
    ]
