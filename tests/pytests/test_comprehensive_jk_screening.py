...
@pytest.mark.parametrize("scf_type",
                         ["PK", "DIRECT", "OUT_OF_CORE", "DISK_DF", "MEM_DF", "DFDIRJ+LINK", "DFDIRJ+COSX"])
@pytest.mark.parametrize("scf_subtype",
                         ["AUTO", "INCORE", "OUT_OF_CORE", "YOSHIMINE_OUT_OF_CORE", "REORDER_OUT_OF_CORE"])
...
def test_comprehensive_jk_screening(scf_type, scf_subtype, screening):
    ...
    if should_throw:
        ...
        assert compare(type(e_info), pytest.ExceptionInfo,
                       f'Singlet {scf_type}({scf_subtype})+{screening}  throws exception')
