...
def test_pdms():
    ...
    ref_opdm = psi4.core.Matrix.from_array([
        np.array([[9.84317436e-01, 4.69564137e-03, 3.68310242e-03], [4.69564137e-03, 2.71595295e-03, -8.89586171e-04],
                  [3.68310242e-03, -8.89586171e-04, 4.24202798e-04]]),
        np.zeros((0, 0)),
        np.array([[7.6893825e-05]]),
        np.array([[7.6893825e-05]]),
        np.zeros((0, 0)),
        np.array([[0.00416892, 0.00440128, 0.00073837], [0.00440128, 0.00473526, 0.00083457],
    ...
    ref_tdm = psi4.core.Matrix.from_array([
        np.array([[1.11543287e-02, 6.66837005e-02, -9.01185592e-03], [-3.95119873e-02, 2.57584925e-02, 4.69185262e-02],
                  [-1.44471817e-03, 7.47375607e-05, 4.47155049e-04]]),
        np.zeros((0, 0)),
        np.array([[-1.17823927e-05]]),
        np.array([[-1.17823927e-05]]),
        np.zeros((0, 0)),
        np.array([[-0.0436292, -0.01418167, -0.00124078], [0.04298612, 0.00671799, 0.00043999],
    ...
    assert psi4.compare_matrices(wfn.get_opdm(2, 1, "A", True),
                                 wfn.get_opdm(1, 2, "A", True).transpose(), 6, "TDM SWAP")
