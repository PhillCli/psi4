...
if __name__ == "__main__":
    psi4.core.be_quiet()
    configs = [
        AdccConfig('adc(1)', 'cc-pvdz', molecules['cn'], 'cn', n_states=4, kind='any', reference='uhf',
                   label='quick'),  # cn-adc1
        AdccConfig('adc(2)', 'cc-pvdz', molecules['cn'], 'cn', n_states=6, kind='any', reference='uhf'),  # cn-adc2
        AdccConfig('adc(1)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet',
                   label='quick'),  # h2o-adc1
        AdccConfig('adc(2)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet'),  # h2o-adc2
        AdccConfig('adc(1)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=2, kind='singlet',
                   label='quick'),  # h2o-adc1-quick
        ...
        AdccConfig('cvs-adc(1)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet',
                   core_orbitals=1),  # h2o-cvs-adc1
        AdccConfig('cvs-adc(2)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet',
                   core_orbitals=1),  # h2o-cvs-adc2
        AdccConfig('cvs-adc(2)-x', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet',
                   core_orbitals=1),  # h2o-cvs-adc2x
        AdccConfig('cvs-adc(2)-x',
                   'cc-pvdz',
                   molecules['h2o'],
                   'h2o',
                   n_states=7,
                   kind='triplet',
                   core_orbitals=1,
                   label='quick'),  # h2o-cvs-adc2x-triplets
        AdccConfig('cvs-adc(3)', 'cc-pvdz', molecules['h2o'], 'h2o', n_states=10, kind='singlet',
                   core_orbitals=1),  # h2o-cvs-adc3
        AdccConfig('adc(2)',
                   'cc-pvdz',
                   molecules['h2o'],
                   'h2o',
        ...
        AdccConfig('adc(2)-x',
                   ...
                   kind='singlet',
                   frozen_virtual=5,
                   label='quick'),  # h2o-fv-adc2x
        AdccConfig('adc(2)', '6-31G', molecules['hf'], 'hf', n_states=5, kind='spin_flip',
                   reference='uhf'),  # hf-adc2-spin-flip
        AdccConfig('adc(2)',
                   'sto-3g',
                   molecules['methyloxirane'],
                   'methyloxirane',
                   n_states=5,
                   kind='singlet',
                   gauge='velocity',
                   label='quick')  # methyloxirane
    ]
    ...
    for conf in configs:
        ...
        scf_accuracy = max(psi4.core.get_option("SCF", "E_CONVERGENCE"), psi4.core.get_option("SCF", "D_CONVERGENCE"))
        ...
        if wfn.frzvpi()[0] > 0:
            fv = wfn.frzvpi()[0]
        state = adcc.run_adc(wfn,
                             method=adc_name,
                             n_states=conf.n_states,
                             kind=conf.kind,
                             core_orbitals=conf.core_orbitals,
                             conv_tol=conv_tol,
                             frozen_core=fc,
                             frozen_virtual=fv)

        props = [
            'excitation_energy', 'oscillator_strength', 'oscillator_strength_velocity', 'state_dipole_moment',
            'transition_dipole_moment', 'rotatory_strength'
        ]
        for pr in props:
