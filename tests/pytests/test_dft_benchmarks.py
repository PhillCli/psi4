...
def dft_bench_systems():
    ang = np.array(
        [[-1.551007, -0.114520, 0.000000], [-1.934259, 0.762503, 0.000000], [-0.599677, 0.040712, 0.000000]]
    )
    ...
    h2o_plus = psi4.core.Molecule.from_arrays(
        geom=oldang, elez=[8, 1, 1], units="Angstrom", mass=oldmass, molecular_charge=1, molecular_multiplicity=2
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            "wM06-D3", 0.4563459267, "6-31G", marks=using("dftd3")
        ),  # https://gitlab.com/libxc/libxc/-/issues/180
