...
ref = {
    "nobas": {
        ...
        "MP2-D DISPERSION CORRECTION GRADIENT": np.array(
            [0.000, 0.000, 0.00264290716623, 0.000, 0.000, -0.00264290716623]
        ).reshape((-1, 3)),
...
for bas in ["cc-pVDZ", "cc-pVTZ"]:
    ...
    ref[bas]["MP2D TOTAL GRADIENT"] = (
        ref[bas]["MP2 TOTAL GRADIENT"] + ref["nobas"]["MP2-D DISPERSION CORRECTION GRADIENT"]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"driver": "energy", "name": "MP2-d", "pv": "MP2D", "options": {}}, id="mp2d-energy", marks=using("mp2d")
        ),
        pytest.param({"driver": "gradient", "name": "Mp2", "pv": "MP2", "options": {}}, id="mp2-gradient"),
        pytest.param(
            {"driver": "gradient", "name": "Mp2", "pv": "MP2", "dertype": 0, "options": {}},
            id="mp2-gradient-findif",
            marks=pytest.mark.findif,
        ),
        pytest.param(
            {"driver": "gradient", "name": "MP2-d", "pv": "MP2D", "options": {}},
            id="mp2d-gradient",
            marks=using("mp2d"),
        ),
        pytest.param(
            {"driver": "gradient", "name": "MP2-d", "pv": "MP2D", "dertype": 0, "options": {}},
...
def test_dft_mp2(inp):
    ...
    assert compare_values(
        ref["nobas"]["NUCLEAR REPULSION ENERGY"], h2.nuclear_repulsion_energy(), 10, "Nuclear Repulsion Energy"
    )
    ...
    for obj in [psi4.core, wfn]:
        ...
        for qcvar in [inp["pv"] + " TOTAL ENERGY", "CURRENT ENERGY"]:
            assert compare_values(
                ref[basisset][inp["pv"] + " TOTAL ENERGY"], obj.variable(qcvar), 10, basisset + " " + qcvar
            )
        ...
        if inp["pv"] == "MP2D":
            for qcvar in ["DISPERSION CORRECTION ENERGY", "MP2D DISPERSION CORRECTION ENERGY"]:
                assert compare_values(
                    ref["nobas"]["MP2-D DISPERSION CORRECTION ENERGY"], obj.variable(qcvar), 10, basisset + " " + qcvar
                )
            ...
            if inp["driver"] == "gradient" and "dertype" not in inp:
                for qcvar in ["DISPERSION CORRECTION GRADIENT", "MP2D DISPERSION CORRECTION GRADIENT"]:
                    assert compare_arrays(
                        ref["nobas"]["MP2-D DISPERSION CORRECTION GRADIENT"],
                        np.asarray(obj.variable(qcvar)),
                        10,
                        basisset + " disp grad",
                    )
    ...
    if inp["driver"] == "gradient":
        for retrn in [grad, wfn.gradient()]:
            atol = 2.0e-8 if "dertype" in inp else 1.0e-10
            assert compare_values(
                ref[basisset][inp["pv"] + " TOTAL GRADIENT"], np.asarray(retrn), basisset + " tot grad", atol=atol
            )
...
def test_mp2d_opt():
    ...
    assert compare_values(1.4259, h2.R, "h2 bond length", atol=1.0e-3)
